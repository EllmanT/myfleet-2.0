...
const DashDriverAnalytics = () => {
  ...
  let driver;
  driver = driversPage && driversPage.find((v) => v._id === driverId);
  ...
  const handleDownloadSLC = (driverId) => {
    const element = document.getElementById("print-content-slc");
    html2pdf().from(element).save(`${driverId}_Overview_LineChart.pdf`);
  };
  const handleDownloadBC = (driverId) => {
    const element = document.getElementById("print-content-bc");
    html2pdf().from(element).save(`${driverId}_Overview_Barchart.pdf`);
  };

  const handleDownloadPC = (driverId) => {
    const element = document.getElementById("print-content-pc");
    html2pdf().from(element).save(`${driverId}_Overview_Piechart.pdf`);
  };

  const handleDownloadMLC = (driverId) => {
    const element = document.getElementById("print-content-mlc");
    html2pdf().from(element).save(`${driverId}_Overview_Multilinechart.pdf`);
  };

  const handleViewMLC = (driverId) => {
    navigate(`/driver-analysis/daily-data/${driverId}`);
  };
      ...
      >
        ...
        >
          <Box display={"flex"}>
            ...
            <IconButton
              sx={{ ml: "0.5rem" }}
              onClick={() => handleDownloadBC(driver.name)}
            >
              <DownloadOutlined />
            </IconButton>
          </Box>
          <DriverBarChart driverId={driverId} view={selectedOptionBC} isDashboard={true} />
        </Box>
        <Box
        ...
        >
          <Box display={"flex"}>
            ...
            <IconButton
              sx={{ ml: "0.5rem" }}
              onClick={() => handleDownloadSLC(driver.name)}
            >
              <DownloadOutlined />
            </IconButton>
          </Box>
        ...
        >
          <Box display={"flex"}>
            ...
            <IconButton
              sx={{ ml: "8rem" }}
              onClick={() => handleViewMLC(driverId)}
            >
              <RemoveRedEyeOutlined />
            </IconButton>
            <IconButton
              sx={{ ml: "0.1rem" }}
              onClick={() => handleDownloadMLC(driver.name)}
            >
              <DownloadOutlined />
            </IconButton>
          </Box>

          <DriverDailyLineChart
            driverId={driverId}
            isDashboard={true}
            view="jobs"
            isDisabled={true}
        ...
        >
          <Box display={"flex"}>
            ...
            <IconButton
              sx={{ ml: "12rem" }}
              onClick={() => handleDownloadPC(driver.name)}
            >
              <DownloadOutlined />
            </IconButton>
          </Box>
...
export default DashDriverAnalytics;
